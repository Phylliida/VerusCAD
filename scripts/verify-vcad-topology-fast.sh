...
MODULE="${1:-runtime_halfedge_mesh_refinement}"
FUNCTION_PATTERN="${2:-}"
...
if [[ $NIX_SHELL_USABLE -eq 1 ]]; then
  nix-shell -p rustup --run "
    ...
    cd '$ROOT_DIR'
    cargo verus verify --manifest-path crates/vcad-topology/Cargo.toml -p vcad-topology --features verus-proofs -- $VERIFY_SCOPE --triggers-mode silent
  "
else
