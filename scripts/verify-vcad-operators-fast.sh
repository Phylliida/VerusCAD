...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'EOF'
usage: ./scripts/verify-vcad-operators-fast.sh [module] [function_pattern]

examples:
  ./scripts/verify-vcad-operators-fast.sh
  ./scripts/verify-vcad-operators-fast.sh <module>
  ./scripts/verify-vcad-operators-fast.sh <module> <function_pattern>
EOF
  exit 0
fi
...
if [[ -n "$FUNCTION_PATTERN" && -z "$MODULE" ]]; then
  echo "error: function_pattern requires module"
  exit 1
fi
...
if [[ -n "$MODULE" && -n "$FUNCTION_PATTERN" ]]; then
...
if [[ $NIX_SHELL_USABLE -eq 1 ]]; then
  nix-shell -p rustup --run "
    ...
    cd '$ROOT_DIR'
    cargo verus verify --manifest-path crates/vcad-operators/Cargo.toml -p vcad-operators --features verus-proofs -- $VERIFY_SCOPE --triggers-mode silent
  "
else
  echo "note: nix-shell unavailable; using direct cargo-verus fallback"
  setup_rustup_shim
  VERIFY_SCOPE_FALLBACK=()
  if [[ -n "$MODULE" ]]; then
    VERIFY_SCOPE_FALLBACK+=(--verify-module "$MODULE")
    if [[ -n "$FUNCTION_PATTERN" ]]; then
      VERIFY_SCOPE_FALLBACK+=(--verify-function "$FUNCTION_PATTERN")
    fi
  fi
  PATH="$SHIM_DIR:$VERUS_SOURCE/target-verus/release:$PATH" \
    ...
    -- "${VERIFY_SCOPE_FALLBACK[@]}" --triggers-mode silent
fi
