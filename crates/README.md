...
5. `vcad-operators`: Euler operators and local mesh rewrites built on topology + geometry invariants.
6. `vcad-kernel`: higher-level modeling operations built on lower verified layers.
7. `vcad-cli`: small executable for examples and local experiments.

Dependency direction should remain one-way:
`vcad-math -> vcad-geometry -> vcad-sketch -> vcad-topology -> vcad-operators -> vcad-kernel -> vcad-cli`.
