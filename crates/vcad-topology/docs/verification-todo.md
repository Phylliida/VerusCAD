...
- [x] `verify-vcad-topology-fast.sh` and `verify-vcad-topology.sh` both green.
  ...
  - burndown update (2026-02-18, exit-condition maintenance rerun + current-pass matrix replay):
    ...
    - warning-scope note:
      ...
      `vcad-geometry`), with no warnings from `vcad-topology`.
    - operational note:
      one feature-test invocation printed
      `Blocking waiting for file lock on build directory` because feature-mode
      tests were launched in parallel; it resumed automatically and completed.
    - failed attempts:
      none in this pass.
    - verification checks:
      `./scripts/verify-vcad-topology-fast.sh`
  ...
  - burndown update (2026-02-18, exit-condition maintenance rerun + current-pass matrix replay):
    ...
    - verification checks:
      ...
      `cargo test -p vcad-topology --features "geometry-checks,verus-proofs"`
      passed (`6 passed, 0 failed`).
  - burndown update (2026-02-18, exit-condition maintenance rerun + matrix refresh):
    - selected task in this pass:
      rerun Exit Condition trust-surface scans and refresh the full
      `vcad-topology` verification/test matrix on the current workspace tree.
    - run timestamp:
      `2026-02-18T07:52:17-08:00`.
    - trust-surface scans:
      - `rg -n "assume_specification|external_fn_specification|\\buninterpreted\\b|admit\\(|assume\\(" crates/vcad-topology/src/runtime_halfedge_mesh_refinement.rs crates/vcad-topology/src/verified_checker_kernels.rs crates/vcad-topology/src/halfedge_mesh.rs`
        returned no matches (`0` lines).
      - `rg -n "\\[verifier::external_body\\]" crates/vcad-topology/src`
  ...
  - burndown update (2026-02-18, exit-condition maintenance rerun + current-pass matrix replay):
    ...
    - verification checks:
      ...
      `cargo test -p vcad-topology --features "geometry-checks,verus-proofs"`
      passed (`6 passed, 0 failed`).
  - burndown update (2026-02-18, exit-condition maintenance rerun + matrix replay):
    - selected task in this pass:
      rerun Exit Condition trust-surface scans and replay the full
      `vcad-topology` verification/test matrix on the current workspace tree.
    - run timestamp:
      `2026-02-18T07:44:16-08:00`.
    - trust-surface scans:
      - `rg -n "assume_specification|external_fn_specification|\\buninterpreted\\b|admit\\(|assume\\(" crates/vcad-topology/src/runtime_halfedge_mesh_refinement.rs crates/vcad-topology/src/verified_checker_kernels.rs crates/vcad-topology/src/halfedge_mesh.rs`
        returned no matches (`0` lines).
      - `rg -n "\\[verifier::external_body\\]" crates/vcad-topology/src`
    ...
    - failed attempts:
      none in this pass.
    - operational note:
      the parallel cargo test fanout printed one transient
      `Blocking waiting for file lock on build directory` line; it resolved
      automatically and all test commands completed successfully.
    - verification checks:
      `./scripts/verify-vcad-topology-fast.sh`
      passed (`192 verified, 0 errors`);
      `./scripts/verify-vcad-topology.sh`
  ...
  - burndown update (2026-02-18, exit-condition maintenance rerun + current-pass matrix replay):
    ...
    - warning-scope note:
      ...
      `vcad-geometry`), with no warnings from `vcad-topology`.
    - operational note:
      parallel `cargo test` invocations briefly printed
      `Blocking waiting for file lock on build directory` and then completed
      successfully.
    - failed attempts:
      none in this pass.
    - verification checks:
      ...
      `cargo test -p vcad-topology --features "geometry-checks,verus-proofs"`
      passed (`6 passed, 0 failed`).
  - burndown update (2026-02-18, exit-condition trust-surface guard integration + matrix replay):
    - selected task in this pass:
      harden Exit Condition maintenance by adding an automated trust-surface
      preflight script to topology verification entrypoints, then replay the
      full `vcad-topology` verification/test matrix.
    - run timestamp:
      `2026-02-18T08:16:07-08:00`.
    - code hardening:
      - added `scripts/check-vcad-topology-trust-surface.sh` to fail fast on
        trust-surface regressions in `crates/vcad-topology/src`:
        `assume_specification`, `external_fn_specification`, `uninterpreted`,
      ...
      - wired the new guard into:
        ...
        `scripts/verify-vcad-topology.sh`.
      - updated `scripts/README.md` to document the new guard and its
        integration into topology verification scripts.
    - trust-surface scans:
      - `./scripts/check-vcad-topology-trust-surface.sh`
        passed; no forbidden trust markers found in
        `crates/vcad-topology/src`, and
  ...
  - burndown update (2026-02-18, exit-condition maintenance rerun + matrix replay refresh):
    ...
    - warning-scope note:
      ...
      `vcad-geometry`), with no warnings from `vcad-topology`.
    - operational note:
      `cargo test -p vcad-topology` printed
      `Blocking waiting for file lock on build directory` once and then
      continued to successful completion.
    - failed attempts:
      none in this pass.
    - verification checks:
  ...
  - burndown update (2026-02-18, exit-condition maintenance rerun + current-pass matrix replay):
    - selected task in this pass:
      rerun the Exit Condition matrix on the current workspace tree to confirm
      trust-surface and full verification/test status remain green.
    - run timestamp:
      `2026-02-18T08:42:34-08:00`.
    - trust-surface scans:
      - `./scripts/check-vcad-topology-trust-surface.sh`
        passed; `external_type_specification` markers remained constrained to
        `runtime_halfedge_mesh_refinement.rs` with count `6`.
  ...
  - burndown update (2026-02-18, exit-condition maintenance rerun + matrix replay):
    - selected task in this pass:
      ...
      `vcad-topology` verification/test matrix on the current workspace tree.
    - run timestamp:
      `2026-02-18T08:33:05-08:00`.
    - trust-surface scans:
      - `./scripts/check-vcad-topology-trust-surface.sh`
        passed; external-type shims remained constrained to
        `runtime_halfedge_mesh_refinement.rs` with count `6`.
    - warning-scope note:
      all `cargo test -p vcad-topology` invocations in this pass emitted
      warnings only from dependency crates (`vstd`, `vcad-math`,
  ...
  - burndown update (2026-02-18, exit-condition maintenance rerun + current-pass matrix replay):
    - selected task in this pass:
      rerun the Exit Condition matrix on the current tree to confirm
      trust-surface and full verification/test status remain green.
    - run timestamp:
      `2026-02-18T08:35:37-08:00`.
    - trust-surface scans:
      - `./scripts/check-vcad-topology-trust-surface.sh`
        passed; `external_type_specification` markers remained constrained to
        `runtime_halfedge_mesh_refinement.rs` with count `6`.
  ...
  - burndown update (2026-02-18, vertex-representative checker completeness revalidation + matrix replay):
    ...
    - failed attempts:
      none in this pass.
  - burndown update (2026-02-18, exit-condition maintenance rerun + direct command replay):
    - selected task in this pass:
      rerun Exit Condition trust-surface validation and replay the full
      `vcad-topology` verification/test matrix with direct entrypoint commands.
    - run timestamp:
      `2026-02-18T08:45:18-08:00`.
    - trust-surface scans:
      - `./scripts/check-vcad-topology-trust-surface.sh`
        passed; `external_type_specification` markers remained constrained to
        `runtime_halfedge_mesh_refinement.rs` with count `6`.
  ...
  - burndown update (2026-02-18, edge-two-half-edges checker revalidation + matrix replay):
    ...
    - verification checks:
      ...
      - `cargo test -p vcad-topology --features "geometry-checks,verus-proofs"`
        passed (`6 passed, 0 failed`).
  - burndown update (2026-02-18, edge-cardinality regression hardening + matrix replay):
    - selected task in this pass:
      harden the completed `check_edge_has_exactly_two_half_edges` boundary
      with an explicit negative runtime regression, then replay the full
      `vcad-topology` verification/test matrix.
    - run timestamp:
      `2026-02-18T08:59:28-08:00`.
    - code hardening:
      in `src/halfedge_mesh.rs`, added
      `edge_with_three_incident_half_edges_fails_edge_cardinality_check`.
      the test builds a minimal mesh where one edge has three incident
      half-edges and asserts:
      - `check_edge_has_exactly_two_half_edges()` is `false`,
      - `check_edge_has_exactly_two_half_edges_via_kernel()` is also `false`
        in `verus-proofs` builds.
    - failed attempts:
      first compile failed due a missing `Vertex.position` initializer and an
      unconditional call to the kernel-bridge helper in non-`verus-proofs`
      builds. fixed by adding `RuntimePoint3::from_ints(0, 0, 0)` and gating
      the kernel assertion with `#[cfg(feature = "verus-proofs")]`.
    - warning-scope note:
      all `cargo test -p vcad-topology` invocations in this pass emitted
      warnings only from dependency crates (`vstd`, `vcad-math`,
      `vcad-geometry`), with no warnings from `vcad-topology`.
    - verification checks:
      `cargo test -p vcad-topology edge_with_three_incident_half_edges_fails_edge_cardinality_check -- --nocapture`
      passed (`1 passed, 0 failed`);
      `./scripts/verify-vcad-topology-matrix.sh`
      passed:
      - trust-surface guard passed,
      - fast verification passed (`192 verified, 0 errors` partial),
      - full verification passed (`227 verified, 0 errors`),
      - `cargo test -p vcad-topology` passed (`7 passed, 0 failed`),
      - `cargo test -p vcad-topology --features geometry-checks` passed
        (`8 passed, 0 failed`),
      - `cargo test -p vcad-topology --features "geometry-checks,verus-proofs"`
