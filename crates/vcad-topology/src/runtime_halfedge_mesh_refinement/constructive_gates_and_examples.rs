...
#[cfg(feature = "geometry-checks")]
#[allow(dead_code)]
pub fn runtime_check_face_coplanarity_seed0_fixed_witness_triangle_or_quad_oriented_full_equivalence_sound_complete_bridge(
    m: &Mesh,
) -> (out: bool)
    requires
        mesh_runtime_all_faces_triangle_or_quad_cycles_spec(m),
        ...
        ),
{
    ...
    proof {
        assert(runtime_sound_complete_ok == m.check_face_coplanarity());
        assert(runtime_sound_complete_ok ==> mesh_valid_spec(m@));
        assert(runtime_sound_complete_ok ==> mesh_runtime_all_faces_coplanar_spec(m));
        assert(runtime_sound_complete_ok ==> mesh_runtime_all_faces_coplanar_seed0_fixed_witness_spec(m));
        assert(runtime_sound_complete_ok ==> mesh_runtime_all_faces_seed0_corner_non_collinear_spec(m));
        assert(runtime_sound_complete_ok ==> mesh_runtime_all_faces_seed0_plane_contains_vertices_spec(m));
        assert(runtime_sound_complete_ok ==> mesh_runtime_all_faces_oriented_seed0_planes_spec(m));
        assert(runtime_sound_complete_ok);
        assert(mesh_valid_spec(m@));
        assert(mesh_structurally_valid_spec(m@));
        assert(mesh_index_bounds_spec(m@));
        assert(mesh_face_next_cycles_spec(m@));
...
#[cfg(feature = "geometry-checks")]
#[allow(dead_code)]
pub fn runtime_check_face_convexity_triangle_projected_turn_complete_from_runtime_with_geometry_and_non_zero_edges_sound_complete_bridge(
    m: &Mesh,
) -> (out: bool)
    requires
        mesh_runtime_geometric_topological_consistency_with_geometry_and_non_zero_edges_spec(m),
...
#[cfg(feature = "geometry-checks")]
#[allow(dead_code)]
pub fn runtime_check_face_coplanarity_seed0_fixed_witness_complete_from_runtime_with_geometry_and_triangle_or_quad_face_sound_complete_bridge(
    m: &Mesh,
) -> (out: bool)
    requires
        mesh_runtime_geometric_topological_consistency_with_geometry_spec(m),
...
{
    let runtime_coplanarity_ok = m.check_face_coplanarity();
    if !runtime_coplanarity_ok {
        return false;
    }
    ...
    proof {
        assert(_complete_ok);
        assert(runtime_coplanarity_ok == m.check_face_coplanarity());
        assert(mesh_runtime_geometric_topological_consistency_seed0_coplanarity_bundle_spec(m));
        assert(mesh_runtime_all_faces_coplanar_spec(m));
        assert(mesh_runtime_all_faces_coplanar_seed0_fixed_witness_spec(m));
        assert(mesh_runtime_all_faces_seed0_corner_non_collinear_spec(m));
        ...
        assert(runtime_coplanarity_ok ==> mesh_runtime_all_faces_coplanar_spec(m)) by {
            if runtime_coplanarity_ok {
                assert(mesh_runtime_all_faces_coplanar_spec(m));
            }
        };
        assert(
            runtime_coplanarity_ok ==> mesh_runtime_all_faces_coplanar_seed0_fixed_witness_spec(m)
        ) by {
            if runtime_coplanarity_ok {
                assert(mesh_runtime_all_faces_coplanar_seed0_fixed_witness_spec(m));
            }
        ...
        assert(
            runtime_coplanarity_ok ==> mesh_runtime_all_faces_seed0_corner_non_collinear_spec(m)
        ) by {
            if runtime_coplanarity_ok {
                assert(mesh_runtime_all_faces_seed0_corner_non_collinear_spec(m));
            }
        ...
        assert(
            runtime_coplanarity_ok ==> mesh_runtime_all_faces_seed0_plane_contains_vertices_spec(m)
        ) by {
            if runtime_coplanarity_ok {
                assert(mesh_runtime_all_faces_seed0_plane_contains_vertices_spec(m));
            }
        ...
        assert(
            runtime_coplanarity_ok ==> mesh_runtime_all_faces_oriented_seed0_planes_spec(m)
        ) by {
            if runtime_coplanarity_ok {
                assert(mesh_runtime_all_faces_oriented_seed0_planes_spec(m));
            }
        ...
        },
{
    ...
    proof {
        ...
        assert(w.api_ok ==> mesh_runtime_all_half_edges_non_zero_geometric_length_spec(m)) by {
            if w.api_ok {
                assert(mesh_runtime_geometric_topological_consistency_with_geometry_and_non_zero_edges_spec(m));
                assert(mesh_runtime_all_half_edges_non_zero_geometric_length_spec(m));
            }
        };
        assert(w.api_ok ==> mesh_runtime_all_faces_coplanar_seed0_fixed_witness_spec(m)) by {
            if w.api_ok {
                assert(mesh_runtime_geometric_topological_consistency_seed0_coplanarity_bundle_spec(m));
                assert(mesh_runtime_all_faces_coplanar_seed0_fixed_witness_spec(m));
            }
        };
        assert(w.api_ok ==> mesh_runtime_all_faces_seed0_corner_non_collinear_spec(m)) by {
            if w.api_ok {
                assert(mesh_runtime_geometric_topological_consistency_seed0_coplanarity_bundle_spec(m));
                assert(mesh_runtime_all_faces_seed0_corner_non_collinear_spec(m));
            }
        };
        assert(w.api_ok ==> mesh_runtime_all_faces_seed0_plane_contains_vertices_spec(m)) by {
            if w.api_ok {
                assert(mesh_runtime_geometric_topological_consistency_seed0_coplanarity_bundle_spec(m));
                assert(mesh_runtime_all_faces_seed0_plane_contains_vertices_spec(m));
            }
        };
        assert(w.api_ok ==> mesh_runtime_all_faces_oriented_seed0_planes_spec(m)) by {
            if w.api_ok {
                assert(mesh_runtime_geometric_topological_consistency_seed0_coplanarity_bundle_spec(m));
                assert(mesh_runtime_all_faces_oriented_seed0_planes_spec(m));
            }
