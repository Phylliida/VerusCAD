...
#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_face_coplanarity_seed0_triangle_or_quad_oriented_full_equivalence_sound_complete_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    assert!(
        mesh_all_faces_are_triangles_or_quads(mesh),
        "triangle/quad seed0 coplanarity oriented/full-equivalence sound+complete bridge parity requires triangle/quad faces in {label}"
    );
    let runtime_coplanarity_ok = mesh.check_face_coplanarity();
    let seed0_triangle_or_quad_oriented_full_equivalence_sound_complete_bridge_ok =
        runtime_check_face_coplanarity_seed0_fixed_witness_triangle_or_quad_oriented_full_equivalence_sound_complete_bridge(
            mesh,
        );
    assert_eq!(
        seed0_triangle_or_quad_oriented_full_equivalence_sound_complete_bridge_ok,
        runtime_coplanarity_ok,
        "triangle/quad seed0 coplanarity oriented/full-equivalence sound+complete bridge parity failed for {label}"
    );
}

#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_face_coplanarity_seed0_triangle_or_quad_runtime_full_spec_parity_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    assert!(
        mesh_all_faces_are_triangles_or_quads(mesh),
        "triangle/quad runtime/full-spec parity bridge requires triangle/quad faces in {label}"
    );
    assert!(
        mesh.is_valid(),
        "triangle/quad runtime/full-spec parity bridge requires phase-4 validity in {label}"
    );
    assert!(
        mesh.check_face_corner_non_collinearity(),
        "triangle/quad runtime/full-spec parity bridge requires face-corner non-collinearity in {label}"
    );

    let runtime_coplanarity_ok = mesh.check_face_coplanarity();
    let exhaustive_full_coplanarity_ok = check_face_coplanarity_exhaustive_face_quadruple_oracle(mesh);
    let runtime_full_spec_parity_ok =
        runtime_check_face_coplanarity_seed0_fixed_witness_triangle_or_quad_runtime_full_spec_parity_bridge(
            mesh,
        );
    assert_eq!(
        runtime_full_spec_parity_ok,
        runtime_coplanarity_ok == exhaustive_full_coplanarity_ok,
        "triangle/quad seed0 coplanarity runtime/full-spec parity bridge diverged from runtime-vs-exhaustive-full-coplanarity parity for {label}"
    );
}

#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_face_coplanarity_seed0_runtime_with_geometry_triangle_or_quad_runtime_full_spec_parity_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    assert!(
        mesh_all_faces_are_triangles_or_quads(mesh),
        "triangle/quad runtime-with-geometry runtime/full-spec parity bridge requires triangle/quad faces in {label}"
    );
    let geometric_sound_bridge_ok = runtime_check_geometric_topological_consistency_sound_bridge(mesh);
    if geometric_sound_bridge_ok {
        let runtime_coplanarity_ok = mesh.check_face_coplanarity();
        let exhaustive_full_coplanarity_ok =
            check_face_coplanarity_exhaustive_face_quadruple_oracle(mesh);
        let runtime_with_geometry_runtime_full_spec_parity_ok =
            runtime_check_face_coplanarity_seed0_fixed_witness_triangle_or_quad_runtime_full_spec_parity_from_runtime_with_geometry_bridge(
                mesh,
            );
        assert_eq!(
            runtime_with_geometry_runtime_full_spec_parity_ok,
            runtime_coplanarity_ok == exhaustive_full_coplanarity_ok,
            "triangle/quad runtime-with-geometry runtime/full-spec parity bridge diverged from runtime-vs-exhaustive-full-coplanarity parity for {label}"
        );
    }
}
...
#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_face_convexity_triangle_projected_turn_runtime_with_geometry_completeness_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    assert!(
        mesh_all_faces_are_triangles(mesh),
        "triangle convexity runtime-with-geometry completeness parity requires triangle faces in {label}"
    );
    let geometric_sound_bridge_ok = runtime_check_geometric_topological_consistency_sound_bridge(mesh);
    let triangle_projected_turn_complete_ok = if geometric_sound_bridge_ok {
        runtime_check_face_convexity_triangle_projected_turn_complete_from_runtime_with_geometry_preconditions(
            mesh,
        )
    } else {
    ...
    assert_eq!(
        triangle_projected_turn_complete_ok, geometric_sound_bridge_ok,
        "triangle convexity runtime-with-geometry completeness parity failed for {label}"
    );
}

#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_face_convexity_triangle_projected_turn_runtime_with_geometry_and_non_zero_edges_completeness_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    assert!(
        mesh_all_faces_are_triangles(mesh),
        "triangle convexity runtime-with-geometry+non-zero-edges completeness parity requires triangle faces in {label}"
    );
    let geometric_sound_bridge_ok = runtime_check_geometric_topological_consistency_sound_bridge(mesh);
    let triangle_projected_turn_complete_ok = if geometric_sound_bridge_ok {
        runtime_check_face_convexity_triangle_projected_turn_complete_from_runtime_with_geometry_and_non_zero_edges_preconditions(
    ...
    assert_eq!(
        triangle_projected_turn_complete_ok, geometric_sound_bridge_ok,
        "triangle convexity runtime-with-geometry+non-zero-edges completeness parity failed for {label}"
    );
}

#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_face_convexity_triangle_projected_turn_runtime_with_geometry_and_non_zero_edges_sound_complete_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    assert!(
        mesh_all_faces_are_triangles(mesh),
        "triangle convexity runtime-with-geometry+non-zero-edges sound+complete parity requires triangle faces in {label}"
    );
    let geometric_sound_bridge_ok = runtime_check_geometric_topological_consistency_sound_bridge(mesh);
    if geometric_sound_bridge_ok {
        let runtime_convexity_ok = mesh.check_face_convexity();
        let sound_complete_ok =
            runtime_check_face_convexity_triangle_projected_turn_complete_from_runtime_with_geometry_and_non_zero_edges_sound_complete_bridge(
                mesh,
            );
        assert_eq!(
            sound_complete_ok, runtime_convexity_ok,
            "triangle convexity runtime-with-geometry+non-zero-edges sound+complete parity failed for {label}"
        );
        assert_eq!(
            mesh.check_face_convexity(),
            mesh.check_face_corner_non_collinearity(),
            "triangle convexity runtime-with-geometry+non-zero-edges sound+complete bridge should preserve convexity/non-collinearity parity for {label}"
        );
    }
}

#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_face_convexity_triangle_projected_turn_phase5_runtime_bundle_completeness_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    assert!(
        mesh_all_faces_are_triangles(mesh),
        "triangle convexity phase5-bundle completeness parity requires triangle faces in {label}"
    );
    let geometric_sound_bridge_ok = runtime_check_geometric_topological_consistency_sound_bridge(mesh);
    let triangle_projected_turn_phase5_bundle_complete_ok =
        runtime_check_face_convexity_triangle_projected_turn_complete_from_phase5_runtime_bundle_sound_bridge(
            mesh,
        );
    assert_eq!(
        triangle_projected_turn_phase5_bundle_complete_ok, geometric_sound_bridge_ok,
        "triangle convexity phase5-bundle completeness parity failed for {label}"
    );
...
#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_geometric_consistency_triangle_or_quad_coplanarity_sound_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    assert!(
        mesh_all_faces_are_triangles_or_quads(mesh),
        "triangle/quad geometric-consistency coplanarity sound bridge parity requires triangle/quad faces in {label}"
    );
    let runtime_geometric_ok = mesh.check_geometric_topological_consistency();
    let triangle_or_quad_coplanarity_sound_bridge_ok =
        runtime_check_geometric_topological_consistency_triangle_or_quad_coplanarity_sound_bridge(
            mesh,
        );
    assert_eq!(
        triangle_or_quad_coplanarity_sound_bridge_ok, runtime_geometric_ok,
        "triangle/quad geometric-consistency coplanarity sound bridge parity failed for {label}"
    );
    if triangle_or_quad_coplanarity_sound_bridge_ok {
        assert!(
            mesh.check_face_coplanarity(),
            "triangle/quad geometric-consistency coplanarity sound bridge should imply runtime coplanarity for {label}"
        );
    }
}
...
#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_face_coplanarity_seed0_triangle_or_quad_phase5_runtime_bundle_completeness_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    assert!(
        mesh_all_faces_are_triangles_or_quads(mesh),
        "triangle/quad seed0 coplanarity phase5-bundle completeness parity requires triangle/quad faces in {label}"
    );
    let geometric_sound_bridge_ok = runtime_check_geometric_topological_consistency_sound_bridge(mesh);
    let triangle_or_quad_phase5_bundle_complete_ok =
        runtime_check_face_coplanarity_seed0_fixed_witness_triangle_or_quad_complete_from_phase5_runtime_bundle_sound_bridge(
            mesh,
        );
    assert_eq!(
        triangle_or_quad_phase5_bundle_complete_ok, geometric_sound_bridge_ok,
        "triangle/quad seed0 coplanarity phase5-bundle completeness parity failed for {label}"
    );
    if triangle_or_quad_phase5_bundle_complete_ok {
        assert!(
            mesh.check_face_coplanarity(),
            "triangle/quad seed0 coplanarity phase5-bundle completeness should imply runtime coplanarity for {label}"
        );
    }
}
...
#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_face_coplanarity_seed0_runtime_with_geometry_triangle_or_quad_sound_complete_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    assert!(
        mesh_all_faces_are_triangles_or_quads(mesh),
        "triangle/quad runtime-with-geometry sound+complete parity requires triangle/quad faces in {label}"
    );
    let geometric_sound_bridge_ok = runtime_check_geometric_topological_consistency_sound_bridge(mesh);
    let runtime_with_geometry_triangle_or_quad_sound_complete_ok = if geometric_sound_bridge_ok {
        runtime_check_face_coplanarity_seed0_fixed_witness_complete_from_runtime_with_geometry_and_triangle_or_quad_face_sound_complete_bridge(
            mesh,
        )
    } else {
    ...
    assert_eq!(
        runtime_with_geometry_triangle_or_quad_sound_complete_ok, geometric_sound_bridge_ok,
        "seed0 coplanarity runtime-with-geometry triangle/quad sound+complete parity failed for {label}"
    );
    if runtime_with_geometry_triangle_or_quad_sound_complete_ok {
        assert!(
            mesh.check_face_coplanarity(),
            "triangle/quad runtime-with-geometry sound+complete bridge should imply runtime coplanarity for {label}"
        );
    }
}
...
#[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
fn assert_face_coplanarity_seed0_oriented_plane_sound_complete_bridge_parity(
    mesh: &Mesh,
    label: &str,
) {
    let runtime_coplanarity_ok = mesh.check_face_coplanarity();
    let oriented_plane_sound_complete_ok = if runtime_coplanarity_ok {
        runtime_check_face_coplanarity_seed0_fixed_witness_complete_from_validity_and_oriented_seed0_plane_sound_complete_bridge(
            mesh,
        )
    } else {
    ...
    assert_eq!(
        oriented_plane_sound_complete_ok, runtime_coplanarity_ok,
        "seed0 coplanarity oriented-plane sound+complete bridge parity failed for {label}"
    );
}
...
fn assert_constructive_phase5_gate_parity(mesh: &Mesh, label: &str) {
    ...
    if mesh_all_faces_are_triangles(mesh) {
        ...
        assert_face_convexity_triangle_projected_turn_runtime_with_geometry_and_non_zero_edges_completeness_bridge_parity(
            mesh,
            label,
        );
        assert_face_convexity_triangle_projected_turn_runtime_with_geometry_and_non_zero_edges_sound_complete_bridge_parity(
            mesh,
            label,
        );
        assert_face_convexity_triangle_projected_turn_phase5_runtime_bundle_completeness_bridge_parity(
            mesh,
            label,
        );
    ...
    if mesh_all_faces_are_triangles_or_quads(mesh) {
        ...
        assert_face_coplanarity_seed0_triangle_or_quad_oriented_full_equivalence_sound_complete_bridge_parity(
            mesh,
            label,
        );
        assert_face_coplanarity_seed0_triangle_or_quad_phase5_runtime_bundle_completeness_bridge_parity(
            mesh,
            label,
        );
        assert_geometric_consistency_triangle_or_quad_coplanarity_sound_bridge_parity(mesh, label);
        assert_face_coplanarity_seed0_runtime_with_geometry_triangle_or_quad_completeness_bridge_parity(
            mesh,
            label,
        );
        assert_face_coplanarity_seed0_runtime_with_geometry_triangle_or_quad_sound_complete_bridge_parity(
            mesh,
            label,
        );
        assert_face_coplanarity_seed0_runtime_with_geometry_triangle_or_quad_runtime_full_spec_parity_bridge_parity(
            mesh,
            label,
        );
        if mesh.is_valid() && mesh.check_face_corner_non_collinearity() {
            assert_face_coplanarity_seed0_triangle_or_quad_runtime_full_spec_parity_bridge_parity(
                mesh,
                label,
            );
        }
...
fn diagnostic_witness_is_real_counterexample(
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn face_coplanarity_seed0_triangle_or_quad_oriented_full_equivalence_sound_complete_bridge_matches_runtime_checker(
    ) {
        let noncoplanar_vertices = vec![
            RuntimePoint3::from_ints(0, 0, 0),
            RuntimePoint3::from_ints(1, 0, 0),
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn face_coplanarity_seed0_triangle_or_quad_runtime_full_spec_parity_bridge_matches_exhaustive_full_coplanarity_parity(
    ) {
        let noncoplanar_vertices = vec![
            RuntimePoint3::from_ints(0, 0, 0),
            RuntimePoint3::from_ints(1, 0, 0),
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn face_convexity_triangle_projected_turn_runtime_with_geometry_completeness_bridge_matches_geometric_sound_bridge(
    ) {
        let collinear_mesh = build_collinear_single_triangle_pair_mesh();
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn face_convexity_triangle_projected_turn_runtime_with_geometry_and_non_zero_edges_completeness_bridge_matches_geometric_sound_bridge(
    ) {
        let collinear_mesh = build_collinear_single_triangle_pair_mesh();
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn face_convexity_triangle_projected_turn_runtime_with_geometry_and_non_zero_edges_sound_complete_bridge_matches_runtime_checker(
    ) {
        let collinear_mesh = build_collinear_single_triangle_pair_mesh();
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn face_convexity_triangle_projected_turn_phase5_runtime_bundle_completeness_bridge_matches_geometric_sound_bridge(
    ) {
        let collinear_mesh = build_collinear_single_triangle_pair_mesh();
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn face_coplanarity_seed0_triangle_or_quad_phase5_runtime_bundle_completeness_bridge_matches_geometric_sound_bridge(
    ) {
        let noncoplanar_vertices = vec![
            RuntimePoint3::from_ints(0, 0, 0),
            RuntimePoint3::from_ints(1, 0, 0),
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn face_coplanarity_seed0_runtime_with_geometry_triangle_or_quad_sound_complete_bridge_matches_geometric_sound_bridge(
    ) {
        let noncoplanar_vertices = vec![
            RuntimePoint3::from_ints(0, 0, 0),
            RuntimePoint3::from_ints(1, 0, 0),
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn face_coplanarity_seed0_runtime_with_geometry_triangle_or_quad_runtime_full_spec_parity_bridge_matches_exhaustive_full_coplanarity_parity(
    ) {
        let noncoplanar_vertices = vec![
            RuntimePoint3::from_ints(0, 0, 0),
            RuntimePoint3::from_ints(1, 0, 0),
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn geometric_consistency_triangle_or_quad_coplanarity_sound_bridge_matches_runtime_checker() {
        let noncoplanar_vertices = vec![
            RuntimePoint3::from_ints(0, 0, 0),
            RuntimePoint3::from_ints(1, 0, 0),
            RuntimePoint3::from_ints(1, 1, 1),
        ...
        for (label, mesh) in fixtures {
            assert!(mesh_all_faces_are_triangles_or_quads(&mesh));
            assert_geometric_consistency_triangle_or_quad_coplanarity_sound_bridge_parity(
                &mesh, label,
            );
        }
    }
    ...
    #[cfg(all(feature = "geometry-checks", feature = "verus-proofs"))]
    #[test]
    fn face_coplanarity_seed0_oriented_plane_sound_complete_bridge_matches_runtime_checker() {
        let noncoplanar_vertices = vec![
            RuntimePoint3::from_ints(0, 0, 0),
            RuntimePoint3::from_ints(1, 0, 0),
            RuntimePoint3::from_ints(1, 1, 1),
