...
- [x] Execute `P4.4 Quaternion` checklist from `docs/vcad-math-higher-dim-todo.md`.
  - progress: quaternion model API/ring/conjugation-norm-inverse surfaces are complete, including full multiplication associativity (`lemma_mul_associative`), conjugate anti-homomorphism (`conj(a*b) = conj(b)*conj(a)` up to semantic equality), quaternion norm scale (`lemma_norm2_scale`), and quaternion norm multiplicativity (`lemma_norm2_mul`). Rotation-facing API/proofs are now in place via `unit_spec`, `pure_vec3_spec`, `vector_part_spec`, `rotate_quat_spec`, `rotate_vec3_spec`, `lemma_rotate_vec3_norm_preserves`, and `lemma_rotate_vec3_composition`.
- [ ] Execute `P4.5 Runtime + refinement rollout` checklist from `docs/vcad-math-higher-dim-todo.md`.
  - progress: all planned runtime families and refinement modules now exist; wrapper theorem coverage has been expanded through quaternion add-associativity/add-zero, multiplication associativity, inverse-identity, distributivity recovery, quaternion `norm2` scale/multiplicativity behavior recovery, quaternion rotation-law recovery (`rotate_vec3` norm preservation + composition), conjugate anti-homomorphism recovery, and real-scalar multiplication recovery (`q*real(s)`/`real(s)*q` vs `scale`), but broader regression-law coverage is still pending.
- [ ] Execute `P4.6 Anti-cheating + quality gates` checklist from `docs/vcad-math-higher-dim-todo.md`.
  - progress: started with quaternion contract hardening; inverse-related preconditions/contracts now use semantic nonzero (`!norm2.eqv_spec(0)`) rather than `.num` representation checks, and norm positivity is expressed as a requires-style semantic lemma. Runtime refinement boundaries now also use semantic zero/nonzero for scalar reciprocal and orientation scale wrappers (2D/3D), with proof-local bridges where underlying model lemmas still use `.num`. The underlying model orientation scale lemmas in `orientation.rs` and `orientation3.rs` were likewise hardened to semantic zero/nonzero contracts, scalar monotonicity/cancellation public wrappers were converted from implication-style to strong `requires` contracts, and remaining weak orientation/orientation3 wrapper lemmas were similarly tightened to strong preconditions.
  - backlog focus areas are tracked in `docs/vcad-math-higher-dim-todo.md` under `P4.6` (trusted-boundary reduction, implication-contract hardening, representation-leak cleanup, and contradiction-end cleanup).
