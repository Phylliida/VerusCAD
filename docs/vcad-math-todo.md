...
- [ ] Execute `P4.6 Anti-cheating + quality gates` checklist from `docs/vcad-math-higher-dim-todo.md`.
  - progress: started with quaternion contract hardening; inverse-related preconditions/contracts now use semantic nonzero (`!norm2.eqv_spec(0)`) rather than `.num` representation checks, and norm positivity is expressed as a requires-style semantic lemma.
